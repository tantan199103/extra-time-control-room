...
test('league taxonomy exposes stable league and team URLs', () => {
  ...
  assert.equal(findTeam('nba', 'los-angeles-clippers').slug, 'la-clippers')
  assert.equal(findTeam('nba', 'new-jersey-nets').slug, 'brooklyn-nets')
  assert.equal(findTeam('nba', 'vancouver-grizzlies').slug, 'memphis-grizzlies')
  assert.equal(findTeam('nba', 'seattle-supersonics').slug, 'oklahoma-city-thunder')
  assert.equal(findTeam('mlb', 'oakland-athletics').slug, 'athletics')
