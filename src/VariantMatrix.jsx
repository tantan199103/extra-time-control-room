...
export default function VariantMatrix({ product, onChange, onOptionsChange, onProductChange }) {
  ...
  const generate = () => {
    try {
      const next = generateVariantMatrix(product)
      onChange(next)
      setSelected([])
      setNotice(`${next.filter(row => row.status !== 'ARCHIVED').length} combinations ready. New rows stay in draft until you activate them.`)
