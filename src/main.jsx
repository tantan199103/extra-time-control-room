...
function ProductPage({ product, products, onAdd, onQuickView, startPersonalized = false, account }) {
  ...
  return <main className="pdp">
    ...
    <div className="pdp__commerce">
      ...
      <div className="pdp__gallery" onScroll={event => setGalleryIndex(Math.round(event.currentTarget.scrollLeft / event.currentTarget.clientWidth))}>{gallery.map((item,index) => <figure key={`${item.id}-${index}`} className={index > 0 && index % 3 === 0 ? 'wide' : ''}>{item.type === 'VIDEO' ? <video src={item.url} controls preload="metadata"/> : <img src={item.url} alt={item.alt || `${product.name} view ${index+1}`} width={item.width || undefined} height={item.height || undefined} loading={index === 0 ? 'eager' : 'lazy'} decoding="async"/>}<span>{String(index+1).padStart(2,'0')} / {String(gallery.length).padStart(2,'0')}</span></figure>)}</div>
