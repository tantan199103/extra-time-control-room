...
export function AdminThemeStudio({ theme, onSave }) {
  ...
  const save = async () => {
    ...
    setNotice(result?.source === 'supabase' ? 'Theme saved to Supabase.' : result?.error ? 'Admin sign-in required to save.' : 'Theme saved in preview mode.')
...
export function AdminCollections({ collections, products, onSave }) {
  ...
  const save = async () => { const result = await onSave?.(draftCollections); setNotice(result?.source === 'supabase' ? 'Collection saved to Supabase.' : result?.error ? 'Admin sign-in required to save.' : 'Collection saved in preview mode.'); window.setTimeout(() => setNotice(''), 2200) }
