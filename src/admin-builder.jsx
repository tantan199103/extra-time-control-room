...
import { menuTargetProblem, normalizeMenuLocation } from './lib/storefront-model'
...
export function AdminThemeStudio({ theme, onSave }) {
  ...
  const [content, setContent] = useState({ eyebrow: 'DROP 01 / EXTRA TIME', headline: 'The minutes nobody forgets.', button: 'Explore the drop', ...(theme.content || {}) })
...
export function AdminMenus({ menus, onSave }) {
  ...
  const save = async () => { const invalid=draftMenus.flatMap(menu=>(menu.items||[]).flatMap(item=>[item,...(item.children||[])])).find(item=>item.visible!==false&&menuTargetProblem(item.target,item.type)); if(invalid){setNotice(`Not saved: ${invalid.label || 'Link'} — ${menuTargetProblem(invalid.target,invalid.type)}`);return} const result = await onSave?.(draftMenus.map(menu=>({...menu,location:normalizeMenuLocation(menu.location)}))); setNotice(result?.source === 'supabase' ? 'Navigation saved to Supabase.' : result?.error ? `Not saved: ${result.error}` : 'Changes kept in this preview only; not published.'); window.setTimeout(() => setNotice(''), 2200) }
  return <main className="admin-page admin-menus-page"><BuilderIntro eyebrow="STOREFRONT / NAVIGATION" title={<>MAKE IT<br /><em>FINDABLE.</em></>} copy="Compose the header and footer journeys without touching a component. Changes preview on desktop and mobile." action="Save navigation" onAction={save}/><div className="admin-menu-workspace"><aside className="admin-menu-list"><div className="admin-builder-panel-head"><span>{draftMenus.length} MENUS</span><button className="admin-text-button" onClick={addMenu}><Plus size={13}/> New menu</button></div>{draftMenus.map(menu => <button key={menu.id} className={menu.id === selectedId ? 'is-active' : ''} onClick={() => setSelectedId(menu.id)}><MenuIcon size={15}/><span><strong>{menu.name}</strong><small>{menu.location}</small></span><Status value={menu.status}/></button>)}<div className="admin-menu-tip"><Link2 size={14}/><span><strong>Verified navigation</strong><small>Visible internal links must match a published storefront route.</small></span></div></aside><section className="admin-menu-editor"><div className="admin-menu-editor__head"><div><p>MENU BUILDER / {selected?.location.toUpperCase()}</p><h2>{selected?.name}</h2></div><div><button className="admin-button admin-button--outline" disabled title="Interactive menu preview is not available yet. The navigation outline is shown below."><Eye size={14}/> Preview · Coming soon</button><button className="admin-button admin-button--dark" onClick={save}><Save size={14}/> Save</button></div></div><div className="admin-menu-location"><label className="admin-builder-field"><span>Menu name</span><input value={selected?.name || ''} onChange={event => updateMenu(menu => ({ ...menu, name: event.target.value }))}/></label><label className="admin-builder-field"><span>Theme location</span><select value={normalizeMenuLocation(selected?.location)} onChange={event => updateMenu(menu => ({ ...menu, location: event.target.value }))}><option value="HEADER">Header</option><option value="FOOTER">Footer</option><option value="FIXED_FOOTER_MOBILE">Fixed footer / mobile</option><option value="MOBILE_DRAWER">Mobile drawer</option></select></label></div><div className="admin-menu-items-head"><span>LINKS / USE ARROWS TO REORDER</span><button className="admin-text-button" onClick={addItem}><Plus size={13}/> Add link</button></div><div className="admin-menu-items">{selected?.items.map((item, index) => { const problem=menuTargetProblem(item.target,item.type); return <div className={`admin-menu-item ${problem&&item.visible?'has-error':''}`} key={item.id}><GripVertical size={15} className="admin-menu-drag"/><div className="admin-menu-item__fields"><input value={item.label} onChange={event => updateItem(item.id, 'label', event.target.value)} aria-label="Menu label"/><div><select value={String(item.type||'PAGE').toUpperCase()} onChange={event => updateItem(item.id, 'type', event.target.value)}><option value="PAGE">Page</option><option value="COLLECTION">Collection</option><option value="PRODUCT">Product</option><option value="ACTION">Action</option><option value="EXTERNAL">External</option></select><input value={item.target} onChange={event => updateItem(item.id, 'target', event.target.value)} aria-label="Menu link"/></div>{problem&&item.visible&&<small className="admin-menu-item__error">{problem}</small>}</div><button className={`admin-visibility ${item.visible ? 'is-on' : ''}`} onClick={() => updateItem(item.id, 'visible', !item.visible)}>{item.visible ? 'Visible' : 'Hidden'}</button><div className="admin-menu-item__actions"><button className="admin-block-icon" onClick={() => moveItem(index, -1)} aria-label="Move link up"><ChevronUp size={13}/></button><button className="admin-block-icon" onClick={() => moveItem(index, 1)} aria-label="Move link down"><ChevronDown size={13}/></button><button className="admin-block-icon" onClick={() => updateMenu(menu => ({ ...menu, items: menu.items.filter(entry => entry.id !== item.id) }))} aria-label="Delete link"><Trash2 size={13}/></button></div>{item.children?.length > 0 && <div className="admin-menu-children">{item.children.map(child => <span key={child.id}>↳ {child.label}</span>)}</div>}</div>})}</div><div className="admin-menu-preview"><div className="admin-builder-panel-head"><span>HEADER PREVIEW</span><span><Monitor size={13}/> Desktop</span></div><div className="admin-menu-preview__bar"><strong>90<sup>+</sup> EXTRA TIME</strong>{selected?.items.filter(item => item.visible).map(item => <span key={item.id}>{item.label}</span>)}<b>BAG (0)</b></div><div className="admin-menu-preview__mobile"><Smartphone size={14}/><strong>90+</strong><span>Menu</span></div></div></section></div><SaveNotice notice={notice}/></main>
}
