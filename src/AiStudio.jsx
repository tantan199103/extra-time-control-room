...
export default function AiStudio({ products = [] }) {
  ...
  const editableFields = useMemo(() => fields.map(field => ({ ...field, previewRegion:normalizePreviewRegion(field.previewRegion) })).filter(field => !['photo','logo','textarea'].includes(field.type) && field.previewRegion), [fields])
