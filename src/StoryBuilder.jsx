...
function BuilderField({ fieldId, template, values, update, error }) {
  ...
  if (field.type === 'photo') return <div className="engine-field engine-field--photo"><div className="engine-field__photo-head"><span>{field.label}</span><small>OPTIONAL</small></div><div className={`engine-photo ${value ? 'has-photo' : ''}`}>{value ? <><img src={value} alt="Your uploaded reference"/><button type="button" onClick={() => update(field.key,'')} aria-label="Remove photo"><X size={14}/><span>REMOVE PHOTO</span></button></> : <label htmlFor={`photo-${fieldId}`}><Plus size={17}/><span>ADD A SMALL PERSONAL REFERENCE</span></label>}<input id={`photo-${fieldId}`} type="file" accept="image/png,image/jpeg,image/webp" onChange={event => { const file = event.target.files?.[0]; if (!file) return; if (!/^image\/(?:png|jpe?g|webp)$/.test(file.type) || file.size > 1800000) { event.target.value = ''; return } const reader = new FileReader(); reader.onload = () => update(field.key, reader.result); reader.readAsDataURL(file) }}/></div><p className="engine-photo-help">One image, cropped into a fixed frame. JPG, PNG or WebP · max 1.8MB.</p></div>
...
export default function StoryBuilder({ onAdd }) {
  ...
  return <main className="engine-page">
    <section className="engine-choose"><div><span>BUILD YOUR STORY</span><h1>ONE ENGINE.<br/>FIVE POINTS OF VIEW.</h1><p>Choose the story system first. Every question after this is defined by that template.</p></div><div className="engine-template-strip">{templates.map(item => <button key={item.id} className={template.id === item.id ? 'is-active' : ''} onClick={() => selectTemplate(item.id)}><small>{item.status === 'DRAFT' ? 'EARLY ACCESS' : `V${item.version}`}</small><strong>{item.name}</strong><span>{item.strapline}</span><i>{item.artworkLock}% fixed</i></button>)}</div></section>
