...
function CatalogRow({ product, onDuplicate, selected = false, onToggle }) {
  ...
  return <article className={`admin-catalog-row ${selected ? 'is-selected' : ''}`}>{onToggle&&<button className={`admin-catalog-row__select ${selected?'is-checked':''}`} onClick={() => onToggle(product.id)} aria-label={`${selected?'Deselect':'Select'} ${product.title || product.name}`}>{selected&&<Check size={12}/>}</button>}<button className="admin-catalog-row__open" onClick={() => go(`/admin/products/${product.id}`)} aria-label={`Edit ${product.title || product.name}`}><span className="admin-product-thumb">{product.image?<img src={product.image} alt=""/>:<Package size={19}/>}</span><span className="admin-product-name"><strong>{product.name}</strong><small>{product.sku || product.subtitle || 'No SKU'}</small><i>{tags.slice(0,3).map(tag=><b key={tag}>{tag}</b>)}</i></span><span className="admin-product-type">{product.productGroup || product.type || '—'}</span><span className="admin-product-price">{money(product.price)}</span><StatusPill value={product.status}/><span className="admin-product-stock">{stock ? `${stock} units` : '—'}</span><span className="admin-product-score"><b>{completeness.percent}%</b><i><em style={{width:`${completeness.percent}%`}}/></i></span><ArrowRight size={16}/></button>{onDuplicate&&<button className="admin-catalog-row__duplicate" onClick={() => onDuplicate(product)} title="Create an unsaved draft copy"><Copy size={14}/><span>Duplicate</span></button>}</article>
}
...
function AdminSettings() {
  ...
  const savePayment = async () => {
    ...
    try {
      ...
      setPayment(result.data);setReadiness(result.readiness);setMappingText(JSON.stringify(result.data.paddle.priceMap || {},null,2));setPaymentNotice(result.readiness?.ready?'Payment settings saved. Server secrets are present; checkout implementation can be enabled after webhook verification.':`Settings saved, but checkout stays off: ${(result.readiness?.missing||[]).join(', ')}`)
