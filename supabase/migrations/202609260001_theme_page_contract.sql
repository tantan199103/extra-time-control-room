-- Persist page-specific settings alongside the published block/content contract.
-- This is additive: old themes continue to read through the storefront fallbacks.
begin;
...
begin
  ...
  v_definition:=jsonb_build_object(
    'blocks',coalesce(theme_payload->'blocks','[]'::jsonb),
    'content',coalesce(theme_payload->'content','{}'::jsonb),
    'pageSettings',coalesce(theme_payload->'pageSettings','{}'::jsonb),
    'pages',coalesce(theme_payload->'pages','[]'::jsonb)
  );
