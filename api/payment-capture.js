...
export default async function handler(request, response) {
  ...
  try {
    ...
    try {
      ...
      await client.from('pod_order_events').insert({ order_id: order.id, event_type: 'PAYMENT_REVIEW_REQUIRED', status: order.status, message: 'The payment provider response was inconclusive. Support will reconcile the capture before fulfillment.', metadata: { provider: 'PAYPAL', stage: 'CAPTURE_RESPONSE', reason: safeText(captureError?.message, 240) }, visible_to_customer: true }).catch(() => {})
    ...
    if (captureAmountMismatch) {
      await client.from('pod_order_events').insert({ order_id: order.id, event_type: 'PAYMENT_REVIEW_REQUIRED', status: order.status, message: 'The provider capture amount or currency did not match the checkout total. Fulfillment is blocked pending review.', metadata: { provider: 'PAYPAL', stage: 'CAPTURE_AMOUNT_MISMATCH', captureAmount: capture.amount, captureCurrency: capture.currency, orderAmount: Number(order.grand_total), orderCurrency: order.currency }, visible_to_customer: true }).catch(() => {})
